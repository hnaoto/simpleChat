...
var ptBaseURI = getptBaseURI();var ptNav2 = {
 ...
 ptNav2.addEvents(); ptNav2.addKeyEvent(); ptNav2.addSearchEvent();   var treeRoot = ptUtil.id(ptNav2.root); var crefList = ptUtil.getElemsByClass("ptnav2cref",treeRoot,"li"); var fluidHP = null; if (crefList) {
 for (var i = 0; i < crefList.length; i++) {
 if (crefList[i].id == ptNav2.crefLiIdPrefix + "PT_FLUID_HOMEPAGE") {
 fluidHP = crefList[i]; break; }
 }
 if (fluidHP){
 var menuFirstChild = ptUtil.getFirstChild(treeRoot); if (menuFirstChild && (fluidHP.id != menuFirstChild.id)) 
 treeRoot.insertBefore(fluidHP, menuFirstChild);  }
 }
 ...
 var keyCode = ptUtil.getKeyCode(evt); var actualKeyCode = keyCode | 0x40; var bIsCtrlKey = ptUtil.isCtrlKey(evt);  if (bIsCtrlKey && !ptUtil.isAltKey(evt) && actualKeyCode == 90) {
 ...
 ptEvent.add(goButton,"click",function (){searchProcessing(1);}); var btnActivate = function () {
 var newHref = goButton.href + "?SEARCH_TEXT=" + encodeURI(document.ptnav2srchform.SEARCH_TEXT.value);  if (typeof document.ptnav2srchform.SEARCH_TYPE != "undefined" &&
 ...
 var selNode = ptUtil.id(ptNav2Info.selectedId); if (selNode) {
 var isCref = ptUtil.isClassMember(selNode,ptNav2Info.branding.cref); if ("ltr" === "ltr") {
